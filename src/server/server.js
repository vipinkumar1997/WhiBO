...
// Initialize Socket.IO with our modular setup
const { io, adminIo } = initializeSockets(server);
...
app.get('/terms', (req, res) => {
    res.sendFile(path.join(__dirname, '../client/html/terms.html'));
});

app.get('/privacy', (req, res) => {
    res.sendFile(path.join(__dirname, '../client/html/privacy.html'));
});
...
app.get('/health', (req, res) => {
    ...
    res.json({
        ...
        metrics: {
            ...
            totalUsers: stats.getStats().totalUsers,
            onlineUsers: stats.getStats().onlineUsers,
            activeChats: stats.getStats().activeChats,
            waitingUsers: users.getWaitingQueue().length
        }
    });
});
...
server.listen(port, host, () => {
    console.log(`✨ WhiBO server running in ${environment} mode`);
